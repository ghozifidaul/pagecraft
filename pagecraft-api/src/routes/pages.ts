...
router.put("/:id/pages/:pageId/story", async (c) => {
  ...
  if (!body.story || typeof body.story !== "string" || body.story.trim().length === 0) {
    return c.json({ error: "story is required and must be a non-empty string" }, 400)
  }
...
router.post("/:id/pages/:pageId/story/regenerate", async (c) => {
  ...
  if (!body.feedback || typeof body.feedback !== "string" || body.feedback.trim().length === 0) {
    return c.json({ error: "feedback is required and must be a non-empty string" }, 400)
  }
  ...
  try {
    ...
    const message = err instanceof Error ? err.message : "Story regeneration failed"
...
router.post("/:id/pages/:pageId/illustration", async (c) => {
  ...
  const latestImagePage = await Pages.getLatestImagePageNumber(c.env.pagecraft_db, bookId)
  ...
  if (page.page_number > 1) {
    const prevPage = await Pages.getPageByBookAndNumber(c.env.pagecraft_db, bookId, page.page_number - 1)
